...
import src.phases.PlacePhase;
import src.phases.PlayerPhase;
...
public class Board {
    ...
    private List<NewPosition> createBoardStructure(){
        NewPosition[][] rings = IntStream.range(0, NUM_RINGS)
                .mapToObj(x -> IntStream.range(0, NUM_POSITIONS_PER_RING)
                        .mapToObj(y -> new NewPosition())
                        .toArray(NewPosition[]::new))
                .toArray(NewPosition[][]::new);
